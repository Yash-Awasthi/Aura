...
flowchart TB
    ...
    subgraph Stored
        ...
        DB[(Room v12<br/>profile + contacts + blocklist)]
...
| T5 | **Accidental exchange while phone is in pocket** | Gesture (cosine-similarity gate at 0.88) or biometric is required before the exchange service is started. Activation requires an explicit in-app tap — there is no background trigger. |
